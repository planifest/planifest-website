...
### The Build Process
The `web-app` element (found in `src/web-app/`) is a static site built with Vite. During the `predev` and `prebuild` phases, markdown files are parsed via Node.js scripts (`build-docs.js`), stripped of agent-specific artifact IDs and metadata, and converted into HTML that the frontend consumes. 

Inter-document markdown links are automatically rewritten to ensure smooth web navigation between pages, and the script dynamically authors a static site map alongside a globally accessible persistent navigation menu.
